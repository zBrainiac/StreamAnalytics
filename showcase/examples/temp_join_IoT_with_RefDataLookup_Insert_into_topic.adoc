...
. create a new kafka topics `sensor6stats` with replication-factor 1 --partitions 1
...
kafka@35b0e660b2de:/opt/kafka$ ./bin/kafka-topics.sh --create --bootstrap-server localhost:9092 --replication-factor 1 --partitions 1 --topic sensor6stats
Created topic sensor6stats.
...
. Before you can execute this query, though, the `sensor6stats_sink` table must be created in SSB, mapping it to the `sensor6stats` Kafka topic.
...
+
Edit the DDL statement and replace the `...` value of the `topic` property with the actual topic name: `sensor6stats`.
+
...
+
. Let’s query the `sensor6stats` topic to examine the data that is being written to it.
+
...
[source,sql]
----
CREATE TABLE `sensor6stats_source` (
    `device_id` BIGINT,
    `device_id` BIGINT,
    `city` STRING,
    `lon` DOUBLE,
...
[source,sql]
----
SELECT * FROM sensor6stats_source ;
----
+
[WARNING]
====
